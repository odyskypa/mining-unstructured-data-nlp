...
def extract_features1(tokens, DB_drug_list, DB_brand_list, DB_group_list, hsdb_list):
    ...
    for k in range(len(tokens)):
        ...
        if t in DB_drug_list:
            tokenFeatures.append("DB_drug=True")
        else:
            tokenFeatures.append("DB_drug=False")
        if t in DB_brand_list:
            tokenFeatures.append("DB_brand=True")
        else:
            tokenFeatures.append("DB_brand=False")
        if t in DB_group_list:
            tokenFeatures.append("DB_group=True")
        else:
            tokenFeatures.append("DB_group=False")
        if t in hsdb_list:
            tokenFeatures.append("hsdb=True")
        else:
            tokenFeatures.append("hsdb=False")
        ...
        if sum(1 for c in t if c.isupper()) == 1:
            tokenFeatures.append("cap=OC")
        elif sum(1 for c in t if c.isupper()) > 1:
            tokenFeatures.append("cap=MTOC")
        else:
            tokenFeatures.append("cap=NOC")
        ...
        if any(c in string.punctuation or c.isdigit() for c in t):
            tokenFeatures.append("char=POSC")
        else:
            tokenFeatures.append("char=NO_POSC")
        ...
        if k > 1: # We are after the second word of the sentence,
            ...
            for x in [1,2]:
                ...
                if t_prev in DB_drug_list:
                    tokenFeatures.append(f"DB_drug_{annot}=True")
                else:
                    tokenFeatures.append(f"DB_drug_{annot}=False")

                if t_prev in DB_brand_list:
                    tokenFeatures.append(f"DB_brand_{annot}=True")
                else:
                    tokenFeatures.append(f"DB_brand_{annot}=False")

                if t_prev in DB_group_list:
                    tokenFeatures.append(f"DB_group_{annot}=True")
                else:
                    tokenFeatures.append(f"DB_group_{annot}=False")

                if t_prev in hsdb_list:
                    tokenFeatures.append(f"hsdb_{annot}=True")
                else:
                    tokenFeatures.append(f"hsdb_{annot}=False")
                ...
                if sum(1 for c in t_prev if c.isupper()) == 1:
                    tokenFeatures.append(f"cap_{annot}=OC")
                elif sum(1 for c in t_prev if c.isupper()) > 1:
                    tokenFeatures.append(f"cap_{annot}=MTOC")
                else:
                    tokenFeatures.append(f"cap_{annot}=NOC")

                if any(c in string.punctuation or c.isdigit() for c in t_prev):
                    tokenFeatures.append(f"char_{annot}=POSC")
                else:
                    tokenFeatures.append(f"char_{annot}=NO_POSC")
            ...
            if t_prev in DB_drug_list:
                tokenFeatures.append(f"DB_drug_prev=True")
            else:
                tokenFeatures.append(f"DB_drug_prev=False")

            if t_prev in DB_brand_list:
                tokenFeatures.append(f"DB_brand_prev=True")
            else:
                tokenFeatures.append(f"DB_brand_prev=False")

            if t_prev in DB_group_list:
                tokenFeatures.append(f"DB_group_prev=True")
            else:
                tokenFeatures.append(f"DB_group_prev=False")

            if t_prev in hsdb_list:
                tokenFeatures.append(f"hsdb_prev=True")
            else:
                tokenFeatures.append(f"hsdb_prev=False")
            ...
            if sum(1 for c in t_prev if c.isupper()) == 1:
                tokenFeatures.append("cap_prev=OC")
            elif sum(1 for c in t_prev if c.isupper()) > 1:
                tokenFeatures.append("cap_prev=MTOC")
            else:
                tokenFeatures.append("cap_prev=NOC")

            if any(c in string.punctuation or c.isdigit() for c in t_prev):
                tokenFeatures.append("char_prev=POSC")
            else:
                tokenFeatures.append("char_prev=NO_POSC")
        ...
        if k < len(tokens) - 2:  # We are before the second last word of the sentence,
            ...
            if t_next in DB_drug_list:
                tokenFeatures.append(f"DB_drug_next=True")
            else:
                tokenFeatures.append(f"DB_drug_next=False")

            if t_next in DB_brand_list:
                tokenFeatures.append(f"DB_brand_next=True")
            else:
                tokenFeatures.append(f"DB_brand_next=False")

            if t_next in DB_group_list:
                tokenFeatures.append(f"DB_group_next=True")
            else:
                tokenFeatures.append(f"DB_group_next=False")

            if t_next in hsdb_list:
                tokenFeatures.append(f"hsdb_next=True")
            else:
                tokenFeatures.append(f"hsdb_next=False")
            ...
            if sum(1 for c in t_next if c.isupper()) == 1:
                tokenFeatures.append("cap_next=OC")
            elif sum(1 for c in t_next if c.isupper()) > 1:
                tokenFeatures.append("cap_next=MTOC")
            else:
                tokenFeatures.append("cap_next=NOC")

            if any(c in string.punctuation or c.isdigit() for c in t_next):
                tokenFeatures.append("char_next=POSC")
            else:
                tokenFeatures.append("char_next=NO_POSC")
